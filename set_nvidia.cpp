...
int set_nvidia (bar_config &my_bar_config) {
    ...
    if ((file.find ("\nnvidia ") != string::npos) || (file.find ("nvidia ") == 0)) {
        space = my_bar_config.device.find_first_of(" ");
        if (space != string::npos) {
            program = my_bar_config.device.substr(0,space);
        } else {
            program = my_bar_config.device;
        }
        if (program.size() != 0) {
            pid = get_pid (program);
            if (pid != -1) {
        ...
        fd = popen (my_bar_config.device.c_str(), "r");
        if (fd == NULL) {
            return_value = 3;
            goto close;
        }
        fgets (buffer, sizeof (buffer), fd);
        if (string_to_int (buffer, my_bar_config.integer))
